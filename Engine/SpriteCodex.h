...
public:
	...
	static void DrawWin(Graphics& gfx, const Vei2& pos);

	// Text for size selection (center origin)
	//
	static constexpr int sizeselWidth = 240;
	static constexpr int sizeselHeight = 56;
	static void DrawSmall(Graphics& gfx, const Vei2& pos);
	static void DrawMedium(Graphics& gfx, const Vei2& pos);
	static void DrawLarge(Graphics& gfx, const Vei2& pos);
};